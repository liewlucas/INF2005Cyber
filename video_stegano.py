...
def decode_string(video):
    ...
    #         raise Exception("Failed to open video file: {}".format(video))

    #     # Extract the audio from the video file.
    #     call(["ffmpeg", "-i", video, "-q:a", "0", "-map", "a", "tmp/audio.mp3", "-y"], stdout=open(os.devnull, "w"), stderr=STDOUT)

    #     # Extract the image frames from the video file.
    #     frame_extraction(video)

    #     # Decode the secret message from the image frames.
    #     secret = []
    #     root = "./tmp/"
    #     for i in range(len(os.listdir(root))):
    #         f_name = "{}{}.png".format(root, i)
...
def main():
    ...
    call(["ffmpeg", "-i", "tmp/video.mov", "-i", "tmp/audio.mp3", "-codec", "copy", "encoded_video.mp4", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    clean_tmp()
if __name__ == "__main__":
    while True:
        ...
        if choice == '1':
            ...
            decode_string(input("Enter video file name WITH extension.\n"))
        else:
            break